...
- **Keep aspect/basic cards with a popularity of …** — *any* by default. Every aspect and basic card carries a 0–10 popularity based on the share of MarvelCDB decklists of the matching aspect, published in the last two years after the card came out, that play it. The score sits on fixed breakpoints: 10 for the staples in 30% or more of decks (Helicarrier, Team Training), 8–9 for the well-known good cards around 15–25% (Hawkeye, Indomitable), 4–5 for ordinary cards at 4–8%, and 1–2 for cards in under 2% of decks. Basic cards compete for slots in every deck and run lower, so they are judged on a proportionally lower scale. Choose a minimum and cards scored below it are set to 0 copies; cards at or above it keep their copies. Hero cards and encounter cards are never affected, and the order list shows each card's score. `python mc_popularity.py` refreshes the scores (it reads MarvelCDB day by day and caches the decklists); `python mc_catalog.py` then folds them into the catalog.
